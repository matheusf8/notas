...
export default function Principal({ onLogout }) {
  ...
  async function carregarNotas() {
    const resp = await fetch("https://notas-u4ox.onrender.com/notas/", {
  ...
  async function salvarNota(nota) {
    if (notaEditando) {
      await fetch(`https://notas-u4ox.onrender.com/notas/${notaEditando.id}`, {
        method: "PUT",
        headers: {
      ...
      await fetch("https://notas-u4ox.onrender.com/notas/", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${getToken()}`,
          "Content-Type": "application/json"
  ...
  async function deletarNota(id) {
    await fetch(`https://notas-u4ox.onrender.com/notas/${id}`, {
      method: "DELETE",
      headers: { "Authorization": `Bearer ${getToken()}` }
    });
    carregarNotas();
